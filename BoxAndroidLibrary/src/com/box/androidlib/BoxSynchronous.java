...
/**
 * Use this class to execute requests <b>synchronously</b> against the Box REST
 ...
 * API. All methods in this class are executed in the invoking thread, and
 * therefore are NOT safe to execute in the UI thread of your application. You
 * should only use this class if you already have worker threads or AsyncTasks
 * that you want to incorporate the Box API into. Otherwise, it is recommended
 * that you use {@link com.box.androidlib.Box}.
 *
...
public class BoxSynchronous {
    ...
    /**
     * Returns an instance of BoxSynchronous with the given API key. You must
     * have an API key to execute requests against the Box API. If you do not
     * already have an API key, create an OpenBox application at
     * https://www.box.net/developers/
     *
     * @param apiKey
     *            The API key of your OpenBox application
    ...
    public final TicketResponseParser getTicket() throws IOException {
        ...
        saxRequest(parser,
            BoxUriBuilder.getBuilder(mApiKey).appendQueryParameter("action", "get_ticket").build());
        return parser;
    }
    ...
    public final UserResponseParser getAuthToken(final String ticket) throws IOException {
        ...
        saxRequest(parser, BoxUriBuilder.getBuilder(mApiKey).appendQueryParameter("ticket", ticket)
            .appendQueryParameter("action", "get_auth_token").build());
        return parser;
    }
     ...
     *             connection problem exists.
     */
    public final UserResponseParser registerNewUser(final String username, final String password)
        throws IOException {
        final UserResponseParser parser = new UserResponseParser();
        saxRequest(parser,
            BoxUriBuilder.getBuilder(mApiKey).appendQueryParameter("action", "register_new_user")
                .appendQueryParameter("login", username).appendQueryParameter("password", password)
                .build());
        return parser;
    }
    ...
    public final String verifyRegistrationEmail(final String email) throws IOException {
        ...
        saxRequest(
            parser,
            BoxUriBuilder.getBuilder(mApiKey)
                .appendQueryParameter("action", "verify_registration_email")
     ...
     *             connection problem exists.
     */
    public final FileResponseParser getFileInfo(final String authToken, final long fileId)
        throws IOException {
        final FileResponseParser parser = new FileResponseParser();
        saxRequest(parser, BoxUriBuilder.getBuilder(mApiKey, authToken, "get_file_info")
            .appendQueryParameter("file_id", String.valueOf(fileId)).build());
        return parser;
    }
     ...
     *             connection problem exists.
     */
    public final FolderResponseParser createFolder(final String authToken,
        final long parentFolderId, final String folderName, final boolean share) throws IOException {
        final FolderResponseParser parser = new FolderResponseParser();
        saxRequest(
            parser,
            BoxUriBuilder.getBuilder(mApiKey, authToken, "create_folder")
                .appendQueryParameter("name", folderName)
                .appendQueryParameter("parent_id", String.valueOf(parentFolderId))
                .appendQueryParameter("share", share ? "1" : "0").build());
        return parser;
    }
     ...
     *            set to {@link com.box.androidlib.Box#TYPE_FILE}
     * @param targetId
     ...
     *             connection problem exists.
     */
    public final String copy(final String authToken, final String type, final long targetId,
        final long destinationId) throws IOException {
        final DefaultResponseParser parser = new DefaultResponseParser();
        saxRequest(
            parser,
            BoxUriBuilder.getBuilder(mApiKey, authToken, "copy")
                .appendQueryParameter("target", type)
                .appendQueryParameter("target_id", String.valueOf(targetId))
                .appendQueryParameter("destination_id", String.valueOf(destinationId)).build());
        return parser.getStatus();
    }
     ...
     *            {@link BoxSynchronous#getAuthToken(String)}
     * @param type
     *            The type of item to be deleted. Set to
     *            {@link com.box.androidlib.Box#TYPE_FILE} or
     ...
     *             connection problem exists.
     */
    public final String delete(final String authToken, final String type, final long targetId)
        throws IOException {
        final DefaultResponseParser parser = new DefaultResponseParser();
        saxRequest(
            parser,
            BoxUriBuilder.getBuilder(mApiKey, authToken, "delete")
                .appendQueryParameter("target", type)
     ...
     *            {@link BoxSynchronous#getAuthToken(String)}
     * @param type
     *            The type of item to be moved. Set to
     *            {@link com.box.androidlib.Box#TYPE_FILE} or
     *            {@link com.box.androidlib.Box#TYPE_FOLDER}
     * @param targetId
     ...
     *             connection problem exists.
     */
    public final String move(final String authToken, final String type, final long targetId,
        final long destinationId) throws IOException {
        final DefaultResponseParser parser = new DefaultResponseParser();
        saxRequest(
            parser,
            BoxUriBuilder.getBuilder(mApiKey, authToken, "move")
                .appendQueryParameter("target", type)
                .appendQueryParameter("target_id", String.valueOf(targetId))
                .appendQueryParameter("destination_id", String.valueOf(destinationId)).build());
        return parser.getStatus();
     ...
     *            {@link BoxSynchronous#getAuthToken(String)}
     * @param type
     *            The type of item to be renamed. Set to
     *            {@link com.box.androidlib.Box#TYPE_FILE} or
     *            {@link com.box.androidlib.Box#TYPE_FOLDER}
     * @param targetId
     ...
     *             connection problem exists.
     */
    public final String rename(final String authToken, final String type, final long targetId,
        final String newName) throws IOException {
        final DefaultResponseParser parser = new DefaultResponseParser();
        saxRequest(
            parser,
            BoxUriBuilder.getBuilder(mApiKey, authToken, "rename")
                .appendQueryParameter("target", type)
                .appendQueryParameter("target_id", String.valueOf(targetId))
                .appendQueryParameter("new_name", newName).build());
        return parser.getStatus();
    }
     ...
     *            {@link com.box.androidlib.Box#SORT_RELEVANCE},
     *            {@link com.box.androidlib.Box#SORT_NAME},
     *            {@link com.box.androidlib.Box#SORT_DATE} or
     *            {@link com.box.androidlib.Box#SORT_SIZE}
     * @param page
     ...
     * @param direction
     *            Set to either {@link com.box.androidlib.Box#DIRECTION_ASC} or
     *            {@link com.box.androidlib.Box#DIRECTION_DESC}
     * @param params
     *            Array of string params that can include
     *            {@link com.box.androidlib.Box#SEARCH_PARAM_SHOW_DESCRIPTION}
     *            and/or {@link com.box.androidlib.Box#SEARCH_PARAM_SHOW_PATH}
     * @return the response parser used to capture the data of interest from the
     *         response. See the doc for the specific parser type returned to
     *         see what data is now available. All parsers implement getStatus()
     *         at a minimum.
     ...
     *             connection problem exists.
     */
    public final SearchResponseParser search(final String authToken, final String query,
        final String sort, final int page, final int perPage, final String direction,
        final String[] params) throws IOException {
        final SearchResponseParser parser = new SearchResponseParser();
        final Uri.Builder builder = BoxUriBuilder.getBuilder(mApiKey, authToken, "search")
            .appendQueryParameter("query", query).appendQueryParameter("sort", sort)
     ...
     *             connection problem exists.
     */
    public final String addToTag(final String authToken, final String type, final long targetId,
        final String[] tagNames) throws IOException {
        final DefaultResponseParser parser = new DefaultResponseParser();
        final Uri.Builder builder = BoxUriBuilder.getBuilder(mApiKey, authToken, "add_to_tag")
            .appendQueryParameter("target", type)
            .appendQueryParameter("target_id", String.valueOf(targetId));
        if (tagNames != null) {
     ...
     *             connection problem exists.
     */
    public final CommentsResponseParser getComments(final String authToken, final String type,
        final long targetId) throws IOException {
        final CommentsResponseParser parser = new CommentsResponseParser();
        saxRequest(
            parser,
            BoxUriBuilder.getBuilder(mApiKey, authToken, "get_comments")
                .appendQueryParameter("target", type)
                .appendQueryParameter("target_id", String.valueOf(targetId)).build());
        return parser;
    }
     ...
     *             connection problem exists.
     */
    public final CommentResponseParser addComment(final String authToken, final String type,
        final long targetId, final String message) throws IOException {
        final CommentResponseParser parser = new CommentResponseParser();
        saxRequest(
            parser,
            BoxUriBuilder.getBuilder(mApiKey, authToken, "add_comment")
                .appendQueryParameter("target", type)
                .appendQueryParameter("target_id", String.valueOf(targetId))
     ...
     *             connection problem exists.
     */
    public final String deleteComment(final String authToken, final long commentId)
        throws IOException {
        final DefaultResponseParser parser = new DefaultResponseParser();
        saxRequest(parser, BoxUriBuilder.getBuilder(mApiKey, authToken, "delete_comment")
            .appendQueryParameter("target_id", String.valueOf(commentId)).build());
        return parser.getStatus();
    }
     ...
     *            is {@link com.box.androidlib.Box#PARAM_NOZIP}
     * @return the response parser used to capture the data of interest from the
     *         response. See the doc for the specific parser type returned to
     *         see what data is now available. All parsers implement getStatus()
     *         at a minimum.
     ...
     *             connection problem exists.
     */
    public final UpdatesResponseParser getUpdates(final String authToken,
        final long beginTimeStamp, final long endTimeStamp, final String[] params)
        throws IOException {
     ...
     *             connection problem exists.
     */
    public final ToggleFolderEmailResponseParser toggleFolderEmail(final String authToken,
        final long folderId, final boolean enable) throws IOException {
        final ToggleFolderEmailResponseParser parser = new ToggleFolderEmailResponseParser();
        saxRequest(
            parser,
            BoxUriBuilder.getBuilder(mApiKey, authToken, "toggle_folder_email")
     ...
     *            {@link com.box.androidlib.Box#TYPE_FILE}
     * @param targetId
     *            The file_id or folder_id of the item
     * @return the response parser used to capture the data of interest from the
     *         response. See the doc for the specific parser type returned to
     ...
     *             connection problem exists.
     */
    public final VersionsResponseParser getVersions(final String authToken, final String type,
        final long targetId) throws IOException {
        final VersionsResponseParser parser = new VersionsResponseParser();
        saxRequest(
            parser,
            BoxUriBuilder.getBuilder(mApiKey, authToken, "get_versions")
                .appendQueryParameter("target", type)
                .appendQueryParameter("target_id", String.valueOf(targetId)).build());
        return parser;
    }
     ...
     *             connection problem exists.
     */
    public final VersionsResponseParser makeCurrentVersion(final String authToken,
        final long versionId) throws IOException {
        final VersionsResponseParser parser = new VersionsResponseParser();
        saxRequest(parser, BoxUriBuilder.getBuilder(mApiKey, authToken, "make_current_version")
            .appendQueryParameter("version_id", String.valueOf(versionId)).build());
        return parser;
    }
     ...
     *             connection problem exists.
     */
    public final String setDescription(final String authToken, final String type,
        final long targetId, final String description) throws IOException {
        final DefaultResponseParser parser = new DefaultResponseParser();
        saxRequest(
            parser,
            BoxUriBuilder.getBuilder(mApiKey, authToken, "set_description")
                .appendQueryParameter("target", type)
                .appendQueryParameter("target_id", String.valueOf(targetId))
                .appendQueryParameter("description", description).build());
        return parser.getStatus();
    }
     ...
     *             connection problem exists.
     */
    public final PublicShareResponseParser publicShare(final String authToken, final String type,
        final long targetId, final String password, final String shareMsg, final String[] emails)
        throws IOException {
     ...
     *            {@link BoxSynchronous#getAuthToken(String)}
     * @param type
     *            The type of item to be unshared. Set to
     *            {@link com.box.androidlib.Box#TYPE_FILE} or
     *            {@link com.box.androidlib.Box#TYPE_FOLDER}
     * @param targetId
     *            The file_id or folder_id of the item
     ...
     *             connection problem exists.
     */
    public final String publicUnshare(final String authToken, final String type, final long targetId)
        throws IOException {
        final DefaultResponseParser parser = new DefaultResponseParser();
        saxRequest(
            parser,
            BoxUriBuilder.getBuilder(mApiKey, authToken, "public_unshare")
                .appendQueryParameter("target", type)
                .appendQueryParameter("target_id", String.valueOf(targetId)).build());
        return parser.getStatus();
    }
     ...
     *             connection problem exists.
     */
    public final String privateShare(final String authToken, final String type,
        final long targetId, final String message, final String[] emails, final boolean notify)
        throws IOException {
        final DefaultResponseParser parser = new DefaultResponseParser();
        final Uri.Builder builder = BoxUriBuilder.getBuilder(mApiKey, authToken, "private_share")
            .appendQueryParameter("target", type)
            .appendQueryParameter("target_id", String.valueOf(targetId))
            .appendQueryParameter("message", message != null ? message : "")
     ...
     */
    public final DefaultResponseParser download(final String authToken, final long fileId,
        final File destinationFile, final Long versionId, final FileDownloadListener listener, final Handler handler)
        throws IOException {
     ...
     * described here: {@see {@link <a href=
     * "http://developers.box.net/w/page/12923951/ApiFunction_Upload-and-Download"
     * >http://developers.box.net/w/page/12923951/ApiFunction_Upload-and-
     * Download}</a>}
     *
     * @param authToken
     *            The auth token retrieved through
     ...
     */
    public final FileResponseParser upload(final String authToken, final String action,
        final File file, final String filename, final long destinationId,
        final FileUploadListener listener, final Handler handler) throws FileNotFoundException, MalformedURLException,
        IOException {
    ...
    /**
     * Executes an Http request and triggers response parsing by the specified
     * parser.
     *
     * @param parser
     *            A BoxResponseParser configured to consume the response and
     *            capture data that is of interest
     * @param uri
